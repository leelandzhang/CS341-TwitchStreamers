...
# Add edges
network::add.edges(mutual, mutualEdgelist$numeric_id_1, mutualEdgelist$numeric_id_2)
...
mutual # These six variables should now be listed as vertex attributes when viewing the summary of the network
```
...
***Hypothesis 8***: Users are likely to form connections that create "2-star" configurations, where one user is connected to two others who are not connected to each other.
...
igraph_options(vertex.size = 5, # Smaller node size
               vertex.color = 'grey', # Node color
               edge.color = 'gray80', # Edge color
               edge.arrow.size = .1,  # Arrow size (if directed)
               vertex.label = NA)     # No vertex labels

# Plot the Mutual Follower network
mutual_igraph <- graph.adjacency(as.matrix.network(mutual)) # Convert to igraph object
mutual_igraph <- set_vertex_attr(mutual_igraph, "language", value = features$language)

# Calculate the layout
net_layout <- layout_with_fr(mutual_igraph) # Spring-embedded layout

# Plot with specified parameters
plot(mutual_igraph, layout = net_layout, edge.color = 'black', vertex.size = 3, vertex.label = NA)
...
```{r}
# ----------------------------------------------------------------------------
# Visualize networks with node size based on descaled_views
# ----------------------------------------------------------------------------

# Adjust node size based on descaled_views
node_sizes <- features$views / max(features$views) * 10 # Scale node sizes

# Plot with node sizes based on descaled_views
plot(mutual_igraph, layout = net_layout, edge.color = 'black', vertex.size = node_sizes, vertex.label = NA)
